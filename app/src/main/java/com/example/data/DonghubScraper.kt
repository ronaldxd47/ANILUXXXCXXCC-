...
object DonghubScraper {
    private const val TAG = "DonghubScraper"
    val DOMAINS = listOf("https://donghub.org", "https://donghub.onl")
    ...
    private suspend fun getDocWithFallback(urlPath: String): Document? {
        return withContext(Dispatchers.IO) {
            ...
            for (domain in DOMAINS) {
                try {
                    ...
                    Log.d(TAG, "Failed connection to domain $domain with path $path: ${e.message}")
                }
            }
            null
        }
    ...
    suspend fun fetchSchedule(): List<ScheduleDay> {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                Log.e(TAG, "Error parsing Donghub schedule", e)
            }
            list
        }
    }
